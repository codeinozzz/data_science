...
st.set_page_config(page_title="Search Samples", layout="wide")
            ...
            with st.spinner("Searching..."):
                try:
                    ...
                    response = requests.post(
                        f"{API_URL}/search", files=files, params=params, timeout=30
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        for i, result in enumerate(data["results"], 1):
                            ...
                            with st.expander(
                                f"#{i} - {result['filename']} ({similarity:.1f}% similar)"
                            ):
                    ...
                    st.error(
                        "Cannot connect to API. Make sure the API is running on http://localhost:8001"
                    )
...
with tab2:
    ...
    upload_file = st.file_uploader(
        "Choose an audio file to upload", type=["mp3", "wav", "flac"], key="upload_file"
    )
        ...
        if upload_file is None:
            ...
            with st.spinner("Processing and uploading..."):
                try:
                    ...
                    response = requests.post(
                        f"{API_URL}/ingest", files=files, params=params, timeout=30
                    )
...
with tab3:
    ...
    with col2:
        filter_cluster = st.selectbox(
            "Filter by cluster (optional)", ["All"] + list(range(0, 10))
        )
        ...
        with st.spinner("Searching..."):
            try:
                ...
                response = requests.get(
                    f"{API_URL}/search-by-filters", params=params, timeout=30
                )
